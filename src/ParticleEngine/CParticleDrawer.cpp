...
void CParticleDrawer::drawParticles(irr::core::matrix4& transform, irr::video::IVideoDriver* driver, bool useOneMaterial)
{
    if (ParticleCount == 0)
        return;
...
void CParticleDrawer::doParticles(const irr::scene::ICameraSceneNode* camera, const irr::core::matrix4& transform, irr::u32 timeMs, irr::f32 diff)
{
    ...
    while (it != Particles.end())
    {
        ...
        else
        {
            ...
            if (idx == 0)
                Box.reset(particle->Position-transform.getTranslation());
            else
                Box.addInternalPoint(particle->Position-transform.getTranslation());
